...
if (!nexacro.InputEventInfo) {
	...
	_pMaskTypeDate.setEditMask = function (mask) {
		if (this.editmask != mask) {
			...
			var cur_buf, cur_mask;
			for (var i = 0, len = mask.length; i < len; i++) {
				cur_buf = mask[i];
				if (cur_buf == "y") {
					this._cnt_editformat_year++;
				}
				else if (cur_buf == "M") {
					this._cnt_editformat_month++;
				}
				else if (cur_buf == "d") {
					this._cnt_editformat_day++;
				}
				else if (cur_buf == "H") {
					this._cnt_editformat_hour++;
				}
				else if (cur_buf == "m") {
					this._cnt_editformat_minute++;
				}
				else if (cur_buf == "s") {
					this._cnt_editformat_second++;
				}
			}

			for (var i = 0, len = type_buf.length; i < len; i++) {
				cur_buf = type_buf[i];
